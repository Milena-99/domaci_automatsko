...
unsigned s(const std::vector<unsigned>& args) { 
    return (args[0]+1)%3;
}
...
int main() {
    ...
    TermPtr sssX = std::make_shared<Term>(FunctionData{"s", {ssX}});
    FormulaPtr equalsX = std::make_shared<Formula>(AtomData{"=", {sssX, x}});
    
    FormulaPtr allEqualsX = std::make_shared<Formula>(QuantifierData{QuantifierData::All, "x", equalsX});

    if(!checkSignature(allEqualsX, L.signature)) {
        std::cout << "Signature mismatch" << std::endl;
    }
    else {
        print(allEqualsX); std::cout << std::endl;
    ...
    FormulaPtr f2 = std::make_shared<Formula>(NegData{existsEqualsX});
    if(!checkSignature(f2, L.signature)) {
        std::cout << "Signature mismatch" << std::endl;
    }
    else {
        print(f2); std::cout << std::endl;

        Valuation val;
        std::cout << evaluate(f2, L, val) << std::endl;
    }
